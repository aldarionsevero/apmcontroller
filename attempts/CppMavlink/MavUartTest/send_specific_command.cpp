...
int serial_send(int serial_fd) {
	...
	//arm/disarm declarations
	char buf_arm[300];
	mavlink_message_t msgArm;
	mavlink_command_long_t cmdArm;
	cmdArm.command = 400;	//arm disarm
	cmdArm.target_system = 1;
	cmdArm.target_component = 250;

	/* send  packet every ~20 ms (~50 Hz) */

	if (choice != '\n')
		printf(
				"\n\nChoose packet to send: a - arm; d - disarm; r - raise throttle; l - lower throttle\n\n");
	choice = getchar();
	//int ichoice = choice - '0';
	switch (choice) {
	case 'a':
		printf("Trying to arm APM:Copter.\n");
		//send heartbeat here
		usleep(20000);
		mavlink_msg_heartbeat_encode(1, 250, &msgHb, &hb);
		len = mavlink_msg_to_send_buffer((uint8_t*) buf_hb, &msgHb);
		write(fd, buf_hb, len);
		tcdrain(fd);
		for (i = 0; i < 5; ++i) {
			usleep(20000);
			cmdArm.confirmation = i;
			printf("before write\n");
			cmdArm.param1 = 1;	//arm
			mavlink_msg_command_long_encode(1, 250, &msgArm, &cmdArm);//uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, const mavlink_command_long_t* command_long
			len = mavlink_msg_to_send_buffer((uint8_t*) buf_arm, &msgArm);
			write(fd, buf_arm, len);
			tcdrain(fd);
			usleep(5500000);
			printf("after write\n");
		}
		break;
	case 'd':
		printf("Trying to disarm APM:Copter.\n");
		//send heartbeat here
		usleep(20000);
		mavlink_msg_heartbeat_encode(1, 250, &msgHb, &hb);
		len = mavlink_msg_to_send_buffer((uint8_t*) buf_hb, &msgHb);
		...
		for (j = 0; j < 5; ++j) {
			usleep(20000);
			cmdArm.confirmation = j;
			printf("before write\n");
			cmdArm.param1 = 0; //disarm
			mavlink_msg_command_long_encode(1, 250, &msgArm, &cmdArm); //uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, const mavlink_command_long_t* command_long
			len = mavlink_msg_to_send_buffer((uint8_t*) buf_arm, &msgArm);
			write(fd, buf_arm, len);
			tcdrain(fd);
		...
		break;
	case 'r':
		printf("Trying to raise throttle APM:Copter.\n");
		break;
	case 'l':
		printf("Trying to lower throttle APM:Copter.\n");
		break;
	default:
		break;
