...
int serial_send(int serial_fd) {
	...
	while (1) {
		...
		arm.target_system = 200;
		arm.target_component = 0;

		/* send from system 200 and component 0 */
		mavlink_msg_set_quad_swarm_roll_pitch_yaw_thrust_encode(200, 0,
				&message, &sp);
		...
		if (loopcounter % 100 == 0) {
			...
			if (loopcounter == 100) {
				...
				for (i = 0; i < 5; ++i) {
					...
					printf("before write\n");
					arm.param1 = 1;
					mavlink_msg_command_long_encode(200, 0, &message2, &arm);//uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, const mavlink_command_long_t* command_long
					unsigned len = mavlink_msg_to_send_buffer((uint8_t*) buf,
							&message2);
					write(fd, buf, len);
					tcdrain (fd);
					printf("after write\n");
